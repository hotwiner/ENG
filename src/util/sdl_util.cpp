...
std::shared_ptr<SDL_Window> sdl_util::make_shared_window(SDL_Window* window)
{
    return std::move(std::shared_ptr<SDL_Window>(window, SDL_DestroyWindow));
}

std::shared_ptr<SDL_Renderer> sdl_util::make_shared_renderer(SDL_Renderer* renderer)
{
    return std::move(std::shared_ptr<SDL_Renderer>(renderer, SDL_DestroyRenderer));
}

std::shared_ptr<SDL_Texture> sdl_util::make_shared_texture(SDL_Texture* texture)
{
    return std::move(std::shared_ptr<SDL_Texture>(texture, SDL_DestroyTexture));
}

std::shared_ptr<SDL_Surface> sdl_util::make_shared_surface(SDL_Surface* surface)
{
...
void sdl_util::draw(SDL_Texture* texture, SDL_Rect* srcR, SDL_Rect* destR)
{
    SDL_RenderCopy(Window::windowRenderer.get(), texture, srcR, destR);
}

void sdl_util::draw(std::string relativePath, SDL_Rect* srcR, SDL_Rect* destR)
{
    SDL_RenderCopy(Window::windowRenderer.get(), load_texture(relativePath).get(), srcR, destR);
}

// draws whole surface
void sdl_util::draw_surface(SDL_Surface* surface, SDL_Rect* srcR, SDL_Rect* destR)
{
    SDL_BlitSurface(surface, srcR, Window::windowSurface.get(), destR);
}

std::shared_ptr<SDL_Texture> sdl_util::load_texture(std::string relativePath)
{
...
std::shared_ptr<SDL_Texture> sdl_util::surface_to_texture(SDL_Surface* surface)
{
    return make_shared_texture(SDL_CreateTextureFromSurface(Window::windowRenderer.get(), surface));
}

std::shared_ptr<SDL_Surface> sdl_util::load_surface(std::string relativePath)
{
    return make_shared_surface(IMG_Load((ROOT + relativePath).c_str()));
}

// Render whole surface
void sdl_util::render_whole(std::shared_ptr<SDL_Surface> surface, std::shared_ptr<SDL_Renderer> renderer)
{
    auto windowTexture = sdl_util::surface_to_texture(surface.get());
...
void sdl_util::render_independent(SDL_Texture* texture, SDL_Rect* srcR, SDL_Rect* destR, bool renderOutsideCam)
{
